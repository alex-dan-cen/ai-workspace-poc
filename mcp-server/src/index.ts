...
function readClinerules(projectRoot: string): string | null {
 ...
 * ────────────────────────────────────────────────────────────── */
async function executeParallelPipeline(raw: unknown) {
  ...
  try {
    ...
    Logger.error("Orchestrator", `Sandbox creation failed`, { error: (err as Error).message });
    throw err;
  }

  // 3. Distill target files in parallel (token optimization).
  const distilled = await Promise.all(
    input.targetFiles.map(async (rel) => {
  ...
  const fanout = await Promise.all(
    input.downstreamServers.map(async (srv) => {
  ...
  const payload = {
    ...
    instructions:
      "Run the parallelFanout MCP tool calls concurrently in the host editor, then synthesize the final patch inside the sandbox worktree path.",
  };
 ...
 * ────────────────────────────────────────────────────────────── */
async function generateTaskMetricsReport(raw: unknown) {
 ...
 * ────────────────────────────────────────────────────────────── */
async function captureBehaviorFeedback(raw: unknown) {
 ...
 * ────────────────────────────────────────────────────────────── */
async function parseAuditLog(raw: unknown) {
 ...
 * ────────────────────────────────────────────────────────────── */
async function handleAgentTool(name: string, raw: unknown) {
  ...
  Telemetry.start(sessionId, i.ticketId);
  let report;
  if (name === "agent_developer") report = await agentDeveloper({ ...i, sessionId });
  else if (name === "agent_reviewer") report = await agentReviewer({ ...i, sessionId });
  else report = await agentRefactor({ ...i, sessionId });
  Telemetry.end(sessionId);
...
const server = new Server(
  { name: "mcp-multi-agent-framework", version: "0.1.0" },
  { capabilities: { tools: {} } },
);
...
const TOOL_DEFS = [
  {
    ...
    description:
      "Primary orchestrator. Reads .clinerules, opens a git worktree sandbox, distills target files, applies learned auto-steering to the prompt, and emits a parallel fan-out plan for downstream MCP servers (GitHub, Jira, SonarQube).",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Reads persistent session telemetry and returns a plain-text CLI invoice block: tokens, tool hops, downstream calls, and USD cost per session.",
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "Persist a rejection or steering instruction as a permanent behavior rule for this project. Auto-injected into future prompts.",
    inputSchema: {
      type: "object",
      properties: {
        projectRoot: { type: "string" },
  ...
  },
  {
    ...
    description:
      "Inspect mcp-framework.log. Filter by ticketId, or return only anomalies (errors and denied commands).",
    inputSchema: {
      type: "object",
      properties: {
        ticketId: { type: "string" },
  ...
  },
  {
    name: "agent_refactor",
    description:
      "Refactor agent: distills the target files and pulls SonarQube quality issues in parallel, then returns a refactor plan respecting the steering profile. targetFiles is optional — defaults to git diff vs main.",
    inputSchema: {
      type: "object",
      properties: {
        projectRoot: { type: "string" },
  ...
  },
  {
    name: "run_squad",
    description:
      "Run multiple preset agents (developer/reviewer/refactor) IN PARALLEL on the same ticket and return all reports together.",
    inputSchema: {
      type: "object",
      properties: {
        projectRoot: { type: "string" },
  ...
  },
  {
    ...
    description:
      "Low-level escape hatch: directly call a tool on a spawned downstream MCP server (github/jira/sonarqube/confluence/chrome). The orchestrator manages the child process and connection pool.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Discover what tools a downstream MCP server exposes. Useful before crafting a downstream_call.",
    inputSchema: {
      type: "object",
      properties: {
        server: { type: "string", enum: ["github", "jira", "sonarqube", "confluence", "chrome"] },
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({ tools: TOOL_DEFS as unknown as never }));
...
main().catch((err) => {
  Logger.error("Boot", "Fatal startup failure", { error: (err as Error).message });
  process.exit(1);
});
